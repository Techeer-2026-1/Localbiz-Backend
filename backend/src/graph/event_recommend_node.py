"""EVENT_RECOMMEND 노드 — PG 정형 + OS events_vector k-NN + LLM Rerank 행사 추천.

검색 흐름 (#110 검색 정확도 고도화 v2 — EVENT_SEARCH와 같은 하이브리드 패턴,
단 추천 사유 강조):
...
불변식 #8: asyncpg $1,$2 바인딩 — f-string SQL 금지
...
사용자의 취향·맥락에 가장 적합한 행사 순서를 매기고, 각 행사의 추천 사유를 작성하세요.
...
JSON으로만 응답하세요:
{
  ...
  "reasons": {
    "0": "index 0 행사를 추천하는 이유를 담은 1-2문장 소개",
    "2": "index 2 행사를 추천하는 이유를 담은 1-2문장 소개"
  }
}

ranked_indices에는 상위 5개 index만 포함하세요.
reasons의 키는 후보 목록의 index를 문자열로 쓰고, 카테고리 적합성·일정·위치·
특징 근거를 들어 추천 이유를 1-2문장으로 작성하세요.
"""
...
_EMPTY_LEAD_LINE = "추천할 만한 행사를 찾지 못했어요."
...
async def _search_pg(
    ...
    try:
        ...
        logger.exception("PG event recommend search failed")
        return []
...
# ---------------------------------------------------------------------------
# LLM Rerank (Gemini Flash) — 순위 재배치 + per-event 추천 사유 동시 생성
# ---------------------------------------------------------------------------
async def _llm_rerank(
    ...
    """Gemini Flash로 행사 후보 순위 재배치 + per-event 추천 사유 생성.
    ...
    Returns:
        ...
        descriptions[i]는 reranked_top5[i]에 정렬된 추천 사유 문자열.
...
async def event_recommend_node(state: dict[str, Any]) -> dict[str, Any]:
    ...
    logger.info(
        "event_recommend: pg=%d, os=%d, merged(pg∪os)=%d, naver=%d, final=%d (district=%s, category=%s)",
        len(pg_events),
        len(os_events),
        merged_count,
        len(naver_events),
