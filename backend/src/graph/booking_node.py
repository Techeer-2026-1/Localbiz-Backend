...
_BOOKING_SYSTEM = (
    ...
    "사용자가 원하는 장소의 예약 방법을 웹 검색으로 찾아서 안내해.\n"
    "기본적으로 예약 링크와 연락처만 안내하고, 사용자가 가격·운영시간 등을 물으면 그것만 추가해.\n"
    "출력 형식:\n"
    "- '**예약 방법**', '**운영 시간 및 가격**', '**주의사항**' 같은 굵은 제목 줄 사용 금지\n"
    "- 예약 방법마다 한 줄씩: '공식 홈페이지: [URL]' / '야놀자: [URL]' / '전화: 번호' 형태\n"
    "- 링크는 1~2개만 (공식 사이트 우선, 플랫폼 1개)\n"
    "- 전화 예약만 가능하면 전화번호만\n"
    "- 확실히 작동하는 링크만. 불확실하면 제외\n"
    "- vertexaisearch.cloud.google.com, grounding-api-redirect 등 내부 URL 절대 사용 금지\n"
    "- 전체 5줄 이내"
)
...
def _text_stream_block(booking_info: str) -> dict[str, Any]:
    return {
        "type": "text_stream",
        "system": (
            "사용자에게 예약 방법을 안내합니다. 아래 규칙을 반드시 지키세요:\n"
            "- URL과 전화번호는 절대 변경하지 마세요\n"
            "- '**예약 방법**', '**운영 시간 및 가격**' 같은 굵은 제목 줄 사용 금지\n"
            "- 예약 방법마다 한 줄씩, '공식 홈페이지: [URL]' 형태로 간결하게\n"
            "- 전체 5줄 이내로 핵심만"
        ),
        "prompt": booking_info,
    }
