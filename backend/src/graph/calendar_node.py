...
규칙:
- 현재 일시(KST): {today}
- start_time은 사용자가 날짜·시간을 대화에서 직접 언급한 경우에만 추출한다.
  예) "5월 30일 오후 2시", "오늘 11시", "내일 저녁", "이번 주 토요일" → start_time 추출.
- 코스 장소의 도착 시간(arrival_time, 예: "11:00 90분")은 코스 이동 순서이지 캘린더 시작 시간이 아니다. start_time으로 절대 사용 금지.
- 날짜·시간 미언급 시: start_time = null (시스템이 사용자에게 재질문).
